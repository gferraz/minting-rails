...
module Mint
  module MoneyAttribute
    ...
    class_methods do
      def money_attribute(name, currency: nil, mapping: nil, currency_via: nil)
        if mapping || currency_via
          amount_currency_attribute(name, currency: currency || 'XXX', mapping:, currency_via:)
        elsif attribute_names.include?("#{name}_currency")
          amount_currency_attribute(name, currency: currency || 'XXX')
        else
          unless currency
            raise ArgumentError,
                  'currency: is required when there is no currency column'
          end

          amount_only_attribute(name, currency:)
        end
      end

      private

      def amount_currency_attribute(name, currency: 'XXX', mapping: nil, currency_via: nil)
        ...
        if mapping
          aggregated = find_amount_currency_attributes(name, mapping:)
        elsif attribute_names.include?(name.to_s)
          attribute(name, :mint_money, currency:)
          normalizes(name, with: parser)
          return
        else
          aggregated = find_amount_currency_attributes(name, mapping: nil)
        end
      ...
      def find_amount_currency_attributes(name, mapping:)
        if mapping.present?
